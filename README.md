# 情侣专属存储空间

一个用于情侣专属照片、视频、文档存储管理的全栈项目。

## 项目结构

```
.
├── client/          # 前端项目（React + Vite）
│   ├── src/
│   │   ├── components/    # 组件
│   │   │   ├── layout/    # 布局组件
│   │   │   └── ui/        # UI 组件（shadcn/ui）
│   │   ├── pages/         # 页面
│   │   ├── router/        # 路由配置
│   │   ├── lib/           # 工具函数
│   │   ├── App.jsx
│   │   └── main.jsx
│   ├── package.json
│   └── vite.config.js
│
└── server/          # 后端项目（Express）
    ├── src/
    │   ├── controllers/   # 控制器
    │   ├── models/        # 数据模型
    │   ├── routes/        # 路由
    │   ├── utils/         # 工具类
    │   └── server.js      # 服务器入口
    ├── package.json
    └── .env.example
```

## 技术栈

### 前端
- **React 18** - UI 框架
- **Vite** - 构建工具
- **Tailwind CSS v3** - 样式框架
- **shadcn/ui** - UI 组件库
- **React Router v6** - 路由管理

### 后端
- **Express** - Web 框架
- **MongoDB** - 数据库
- **Mongoose** - ODM
- **CORS** - 跨域支持
- **Multer** - 文件上传（已配置，待使用）

## 快速开始

### 前置要求
- Node.js >= 16.0.0
- MongoDB（本地或远程）
- npm 或 yarn

### 安装依赖

#### 前端
```bash
cd client
npm install
```

#### 后端
```bash
cd server
npm install
```

### 环境配置

1. 复制后端环境变量示例文件：
```bash
cd server
cp env.example .env
```

2. 根据需要修改 `.env` 文件中的配置：
```env
PORT=3001
MONGODB_URI=mongodb://localhost:27017/couple-media
```

### 启动项目

#### 方法一：同时启动前后端（推荐）✨

在项目根目录执行：

```bash
# 首次使用需要安装根目录依赖
npm install

# 启动开发环境（同时启动前后端）
npm run dev
```

这会同时启动：
- 后端服务：`http://localhost:3001`
- 前端应用：`http://localhost:3000`

#### 方法二：分别启动（两个终端）

**终端 1 - 启动后端：**
```bash
cd server
npm start
# 或使用开发模式（自动重启）
npm run dev
```

**终端 2 - 启动前端：**
```bash
cd client
npm run dev
```

> 💡 **提示**：详细启动说明请查看 [START.md](./START.md)

### 构建生产版本

#### 前端
```bash
cd client
npm run build
```

构建产物将输出到 `client/dist` 目录

## 功能模块

### 前端页面
- ✅ 首页 - 数据概览
- ✅ 照片管理
- ✅ 视频管理
- ✅ 文档管理
- ✅ 相册管理
- ✅ 最近访问
- ✅ 收藏夹

### 后端接口

#### 认证相关 (`/api/auth`)
- `POST /api/auth/register` - 用户注册
- `POST /api/auth/login` - 用户登录
- `POST /api/auth/logout` - 用户登出
- `POST /api/auth/refresh` - 刷新令牌

#### 用户相关 (`/api/users`)
- `GET /api/users/me` - 获取当前用户信息
- `PUT /api/users/me` - 更新用户信息
- `GET /api/users` - 获取用户列表
- `GET /api/users/:id` - 获取指定用户信息

#### 文件相关 (`/api/files`)
- `POST /api/files/upload` - 上传文件
- `GET /api/files` - 获取文件列表
- `GET /api/files/type/:type` - 根据类型获取文件（photo/video/document）
- `GET /api/files/:id` - 获取文件详情
- `PUT /api/files/:id` - 更新文件信息
- `DELETE /api/files/:id` - 删除文件
- `POST /api/files/:id/favorite` - 收藏/取消收藏文件

#### 相册相关 (`/api/albums`)
- `POST /api/albums` - 创建相册
- `GET /api/albums` - 获取所有相册
- `GET /api/albums/:id` - 获取相册详情
- `PUT /api/albums/:id` - 更新相册信息
- `DELETE /api/albums/:id` - 删除相册
- `POST /api/albums/:id/files` - 向相册添加文件
- `DELETE /api/albums/:id/files/:fileId` - 从相册移除文件

## 数据模型

### User（用户）
- email - 邮箱（唯一）
- password - 密码
- name - 姓名
- avatar - 头像
- role - 角色（user/admin）
- isActive - 是否激活

### File（文件元数据）
- name - 文件名
- originalName - 原始文件名
- type - 文件类型（photo/video/document）
- mimeType - MIME 类型
- size - 文件大小
- url - 文件URL
- thumbnailUrl - 缩略图URL
- description - 描述
- tags - 标签
- isFavorite - 是否收藏
- uploadedBy - 上传用户ID
- albumId - 所属相册ID
- duration - 视频时长（秒）
- width - 宽度
- height - 高度
- pageCount - PDF 页数

### Album（相册）
- name - 相册名称
- description - 描述
- coverImage - 封面图片
- files - 文件ID数组
- createdBy - 创建用户ID
- isPrivate - 是否私有
- tags - 标签

## 开发说明

### 当前状态
- ✅ 项目基础架构已搭建完成
- ✅ 前端页面布局和路由已配置
- ✅ 后端路由和控制器占位已创建
- ✅ 数据模型已定义
- ⚠️ 所有接口目前仅返回占位数据，业务逻辑待实现

### 下一步开发建议
1. 实现用户认证（JWT）
2. 实现文件上传功能
3. 实现文件列表查询和筛选
4. 实现相册管理功能
5. 添加权限控制
6. 实现文件预览功能

## 许可证

MIT

# couple-media-website
