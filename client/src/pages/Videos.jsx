    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">管理您的视频</p>
        </div>
        <Button onClick={() => setShowUpload(!showUpload)}>
          <Upload className="h-4 w-4 mr-2" />
          {showUpload ? '取消上传' : '上传视频'}
        </Button>
      </div>
      ...
      {showUpload && (
        <div className="space-y-4">
          ...
          <FileUpload fileType="video" onUploadSuccess={handleUploadSuccess} />
        </div>
      )}

      <div>
        <h2 className="text-xl font-semibold mb-4">所有视频</h2>
        <FileList
          key={refreshKey}
          fileType="video"
